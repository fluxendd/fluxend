...
type FileImportService interface {
	ImportCSV(file multipart.File) ([]models.Column, [][]string, error)
}
...
func (s *FileImportServiceImpl) ImportCSV(file multipart.File) ([]models.Column, [][]string, error) {
	// Parse CSV
...
func (s *FileImportServiceImpl) determineColumns(headers []string, dataRows [][]string) ([]models.Column, error) {
	columns := make([]models.Column, 0, len(headers))
